...
command_type_t get_command_type( char *str )
{
    ...
    else if ( 0 == memcmp( str, MKDIR_COMMAND_STR, strlen(MKDIR_COMMAND_STR))){
        ret = COMMAND_MKDIR;
    }
    else if ( 0 == memcmp( str, RM_COMMAND_STR, strlen(RM_COMMAND_STR))){
        ret = COMMAND_RM; 
    }
    else if ( 0 == memcmp( str, LPWD_COMMAND_STR, strlen(LPWD_COMMAND_STR))){
        ret = COMMAND_LPWD;
    }
	else if( 0 == memcmp(str, HELP_COMMAND_STR, strlen(HELP_COMMAND_STR))){
		ret = COMMAND_HELP;
	}
